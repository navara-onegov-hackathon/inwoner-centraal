...
import type {
  StappenplanRow as StappenplanRowModel,
  StappenplanTabId,
} from '../../lib/mapOverzichtToStappenplanTabs';
import { completeTask } from '../../lib/overzichtState';
import type { OverzichtResponse, ResolutionOption, Taak } from '../../types/overzicht';
    ...
    : taak
      ? taak.state === 'blocked'
        ...
        : taak.handled_by === 'us'
          ...
          : 'Nog te doen'
      : null;
  const statusClassName = row.completed
    ? 'text-green-700'
    : taak?.state === 'blocked'
      ? 'text-gray-600'
      : 'text-[#007AC8]';
...
function TaakExpandedDetail({
  ...
  const isUserActionTab = activeTab === 'urgent' || activeTab === 'nog-te-doen';
    ...
    <div className="space-y-4">
      <div className="mb-4 flex flex-wrap items-start justify-between gap-4">
        ...
        {showCompleteButton && (
          <button
            type="button"
            onClick={() => completeWithPatch()}
            className="inline-flex shrink-0 items-center gap-2 rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-800 hover:bg-gray-50"
          >
      ...
      {!isUserActionTab &&
        taak.resolution_options &&
        taak.resolution_options.length > 0 && (
          <ResolutionOptions
